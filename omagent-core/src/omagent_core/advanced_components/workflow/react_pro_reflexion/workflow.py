...
class ReactProReflexionWorkflow(ConductorWorkflow):
    def __init__(self):
        super().__init__(name='react_pro_reflexion')
    ...
    def _configure_tasks(self):
        ...
        self.reflect_task = simple_task(
            ...
            inputs={
                'query': self.query,
                'action_output': self.react_output_task.output('output'),
                'workflow_id': self.id
            }
        )
    ...
    def _configure_workflow(self):
        ...
        # 1. React loop (think -> action -> wiki_search)
        # 2. React output
        # 3. Reflect if needed
        self >> self.react_loop_task >> self.react_output_task >> self.reflect_task 